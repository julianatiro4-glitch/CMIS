...
@section('title', 'Open Technical Support Ticket')
@section('subtitle', 'Report an issue and log a technical support ticket')
...
<div class="max-w-xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow border border-slate-100 p-6">
        <form action="{{ route('technical_support.store') }}" method="POST">
            @csrf
            ...
            <div class="mb-4">
                ...
                <select name="status" id="status" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-navy @error('status') border-red-500 @enderror" required>
                    ...
                    <option value="for_checking" {{ old('status') == 'for_checking' ? 'selected' : '' }}>For Checking</option>
            ...
            <div class="mb-6">
                ...
                <textarea name="issue_problem" id="issue_problem" rows="4" placeholder="Detailed description of the issue..." 
                          class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-navy @error('issue_problem') border-red-500 @enderror" required>{{ old('issue_problem') }}</textarea>
                @error('issue_problem')
                    <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="bg-slate-50 border border-slate-100 rounded-lg p-4 mb-6">
                ...
                <div>
                    ...
                    <textarea name="action_taken" id="action_taken" rows="3" placeholder="Action taken to fix the issue..." 
                              class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-navy @error('action_taken') border-red-500 @enderror">{{ old('action_taken') }}</textarea>
                    @error('action_taken')
                        <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                </div>
