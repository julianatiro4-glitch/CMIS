...
<head>
    ...
    <title>@yield('title', 'CMIS') — QC PESO</title>
...
<aside class="w-60 min-h-screen bg-navy-dark flex flex-col fixed top-0 left-0 z-30 shadow-xl" style="background:#091d42;">
    ...
    <div class="px-4 py-5 border-b border-white/10">
        <div class="flex items-center gap-3">
            <img src="{{ asset('images/logo.png') }}" alt="QC PESO Logo"
    ...
    <nav class="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        ...
        <a href="{{ route('technical_support.index') }}" class="nav-link {{ request()->routeIs('technical_support.*') ? 'active' : '' }}">
            <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
    ...
    <div class="px-3 py-4 border-t border-white/10">
        <div class="flex items-center gap-3 bg-white/5 rounded-lg px-3 py-2.5">
            ...
            <div class="flex-1 min-w-0">
                ...
                <p class="text-blue-300 text-xs truncate">{{ ucfirst(str_replace('_', ' ', auth()->user()->role)) }}</p>
            </div>
...
<div class="flex-1 ml-60 flex flex-col min-h-screen">
    ...
    <header class="bg-white border-b border-slate-200 px-8 py-3.5 flex items-center justify-between sticky top-0 z-20 shadow-sm">
        <div>
            <h1 class="text-sm font-bold text-slate-800">@yield('title', 'Computer Management Inventory System')</h1>
            <p class="text-xs text-slate-400 mt-0.5">@yield('subtitle', 'Quezon City Public Employment Service Office')</p>
        </div>
        <div class="flex items-center gap-3">
            <div class="text-right">
                <p class="text-xs font-medium text-slate-600">{{ now()->format('l') }}</p>
                <p class="text-xs text-slate-400">{{ now()->format('F d, Y') }}</p>
            </div>
            @auth
    ...
    <footer class="px-8 py-3 border-t border-slate-200 bg-white">
        <p class="text-xs text-slate-400 text-center">
            © {{ date('Y') }} Quezon City Public Employment Service Office — Computer Management Inventory System
        </p>
    </footer>
