...
<?php $__env->startSection('subtitle', 'All registered office computers and equipment'); ?>

<?php $__env->startSection('content'); ?>

<form id="bulk-delete-form" action="<?php echo e(route('assets.bulk-destroy')); ?>" method="POST" class="hidden" onsubmit="return confirm('Are you sure you want to delete the selected assets simultaneously?');">
    <?php echo csrf_field(); ?>
</form>
...
<div class="grid grid-cols-1 gap-4 mb-6">
    <div class="bg-white rounded-xl px-5 py-4 shadow-sm border border-slate-100 flex items-center gap-4">
        ...
        <div><p class="text-2xl font-bold text-slate-800"><?php echo e($stats['total']); ?></p><p class="text-xs text-slate-400">Total Assets</p></div>
    </div>
</div>
...
<div class="bg-white rounded-xl shadow-sm border border-slate-100">
    ...
    <div class="flex items-center justify-between px-5 py-4 border-b border-slate-100">
        ...
        <div class="flex items-center gap-2">
            ...
            <?php endif; ?>
            <a href="<?php echo e(route('assets.trash')); ?>" class="flex items-center gap-1.5 text-xs font-medium text-slate-600 border border-slate-200 px-3 py-1.5 rounded-lg hover:bg-slate-50 transition-colors">
            ...
            <a href="<?php echo e(route('assets.export', request()->query())); ?>" class="flex items-center gap-1.5 text-xs font-medium text-slate-600 border border-slate-200 px-3 py-1.5 rounded-lg hover:bg-slate-50 transition-colors">
            ...
            <?php if(auth()->user()->canManage()): ?>
            <a href="<?php echo e(route('assets.create')); ?>" class="flex items-center gap-1.5 text-xs font-semibold text-white px-4 py-1.5 rounded-lg hover:opacity-90" style="background:#0d2a5e;">
    ...
    <form method="GET" class="flex flex-wrap items-center gap-3 px-5 py-3 bg-slate-50 border-b border-slate-100">
        ...
        <select name="division_id" class="border border-slate-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
            ...
            <?php $__currentLoopData = $divisions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $d): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <option value="<?php echo e($d->id); ?>" <?php if(request('division_id') == $d->id): echo 'selected'; endif; ?>>
                    <?php echo e($d->code ? "[{$d->code}] " : ''); ?><?php echo e($d->name); ?>
        ...
        <?php if(request()->hasAny(['q','division_id'])): ?>
            <a href="<?php echo e(route('assets.index')); ?>" class="text-xs text-red-500 hover:underline">Clear</a>
        <?php endif; ?>
    </form>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition-colors">
                    <?php if(auth()->user()->canManage()): ?>
                    <td class="px-4 py-3">
                        <input type="checkbox" name="ids[]" value="<?php echo e($asset->id); ?>" form="bulk-delete-form" class="asset-checkbox rounded border-slate-300 text-navy focus:ring-navy w-4 h-4 cursor-pointer">
                    </td>
                    <?php endif; ?>
                    <td class="px-4 py-3">
                        <p class="font-mono text-xs font-bold text-slate-700 bg-slate-100 px-2 py-0.5 rounded inline-block"><?php echo e($asset->asset_tag); ?></p>
                        <p class="text-xs font-medium text-slate-800 mt-0.5"><?php echo e($asset->name); ?></p>
                        <p class="text-xs text-slate-400"><?php echo e($asset->model ?: ''); ?></p>
                    </td>
                    <td class="px-4 py-3 text-xs text-slate-600">
                        <?php if($asset->division): ?>
                            <?php if($asset->division->code): ?>
                                <span class="font-mono font-bold text-blue-700 bg-blue-50 px-1.5 py-0.5 rounded text-xs"><?php echo e($asset->division->code); ?></span><br>
                            <?php endif; ?>
                            <span class="text-slate-500"><?php echo e($asset->division->name); ?></span>
                        <?php else: ?>
                            <span class="text-slate-300">—</span>
                        <?php endif; ?>
                    </td>
                    <td class="px-4 py-3 text-xs">
                        <span class="px-2 py-0.5 rounded-full font-bold <?php echo e($conditionColors[$asset->condition ?? 'good'] ?? 'bg-slate-100 text-slate-500'); ?>">
                            <?php echo e(ucfirst(str_replace('_', ' ', $asset->condition ?? 'good'))); ?>

                        </span>
                    </td>
                    <td class="px-4 py-3 text-xs text-slate-600 max-w-32">
                        <span title="<?php echo e($asset->cpu); ?>"><?php echo e($asset->cpu ? \Illuminate\Support\Str::limit($asset->cpu, 20) : '—'); ?></span>
                    </td>
                    <td class="px-4 py-3 text-xs text-slate-600 whitespace-nowrap"><?php echo e($asset->ram_total ?: '—'); ?></td>
                    <td class="px-4 py-3 text-xs text-slate-600 whitespace-nowrap"><?php echo e($asset->storage_capacity ?: '—'); ?></td>
                    <td class="px-4 py-3 text-xs text-slate-600 max-w-28">
                        <span title="<?php echo e($asset->operating_system); ?>"><?php echo e($asset->operating_system ? \Illuminate\Support\Str::limit($asset->operating_system, 15) : '—'); ?></span>
                    </td>
                    <td class="px-4 py-3 text-xs text-slate-600"><?php echo e($asset->utilized_by ?: '—'); ?></td>
    ...
    <div class="flex items-center justify-end gap-2">
        ...
        <form action="<?php echo e(route('assets.destroy', $asset)); ?>" method="POST" class="inline"
              onsubmit="return confirm('Delete <?php echo e(addslashes($asset->asset_tag)); ?> — <?php echo e(addslashes($asset->name)); ?>?');">
            <?php echo csrf_field(); ?>
            <?php echo method_field('DELETE'); ?>
            <button type="submit"
                    title="Delete <?php echo e($asset->asset_tag); ?>"
        ...
        <?php endif; ?>
                ...
                <tr>
                    <td colspan="<?php echo e(auth()->user()->canManage() ? '10' : '9'); ?>" class="px-5 py-14 text-center">
                        ...
                        <?php if(auth()->user()->canManage()): ?>
                        <a href="<?php echo e(route('assets.create')); ?>" class="inline-block mt-3 text-xs font-semibold text-white px-4 py-2 rounded-lg" style="background:#0d2a5e;">+ Add First Asset</a>
                        <?php endif; ?>
                    </td>
                </tr>
                <?php endif; ?>
    ...
    <div class="px-5 py-4 border-t border-slate-100"><?php echo e($assets->links()); ?></div>
    <?php endif; ?>
</div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\JulianaTiro\cmis\resources\views/assets/index.blade.php ENDPATH**/ ?>